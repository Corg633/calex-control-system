...
# = STREAMING_CHUNK:Configuring timing and cycling frequencies
CAN_HZ = 10.0          
PRINT_HZ = 1.0          
LOG_HZ = 10.0          

RUN_TIME = 5   #0.5      # Duration of the 60A pulse
DEAD_TIME = 0.001      # Hardware relay switch time

# = STREAMING_CHUNK:Defining hardware targets and limits
# BMS Wake-Up Settings
BMS_WAKE_LSV = 33.0     
BMS_WAKE_AMP = 0.1      
BMS_WAKE_TIME = 0.0     

# Normal Operation Settings (PULSE LIMITS)
# Buck (Charging Battery)
BUCK_HSV = 42.0         
BUCK_LSV = 35.0         # 12S LTO Absolute Max - Provides headroom for 60A
BUCK_AMP = 22.5       # Amps into the LTO Pack

# Boost (Discharging Battery into Resistor)
BOOST_HSV = 52.0        # Pushing up to 52V into the 48V node
BOOST_LSV = 22.0 - 2.0  # 24.0 - 2.0: Drops target LSV so Calex pulls current
BOOST_AMP = 20.0        # Amps pulled from the LTO Pack

# = STREAMING_CHUNK:Setting up CAN database and directories
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
DBC_PATH = os.path.join(SCRIPT_DIR, 'CALEX_DCDC_Database_BCE-24V_V4.dbc')
LOG_DIR = os.path.join(SCRIPT_DIR, 'logs')
if not os.path.exists(LOG_DIR): os.makedirs(LOG_DIR)
CSV_FILENAME = os.path.join(LOG_DIR, f"calex_cycle_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
...
# = STREAMING_CHUNK:Initializing hardware GPIO and CAN functions
def setup_gpio():
...
# = STREAMING_CHUNK:Executing pre-charge and DSP boot sequence
setup_gpio()
...
try:
    # CAN Min values from protocol to avoid 6_CAN_OOR fault
...
# = STREAMING_CHUNK:Triggering JKBMS MOSFET wake-up routine
print(f"\n[BOOT] 4. Pushing {BMS_WAKE_LSV}V to trigger JKBMS MOSFETs...")
...
print("[BOOT] Initialization complete. Transitioning to BUCK/BOOST Cycle mode.")

# = STREAMING_CHUNK:Setting up state machine constants
S_BUCK = 0
S_DEAD_1 = 1
S_BOOST = 2
S_DEAD_2 = 3
...
# = STREAMING_CHUNK:Running primary cyclic state machine
    ...
    try:
        while True:
            ...
            if active_faults != "None":
                ...
                # Hold the state but disable the run command to clear faults safely
                send_command(run=False, direction=0, hs_v=48.0, ls_v=24.0, ls_curr=0.0)
                time.sleep(0.5) 
                # Reset the transition timer so we don't immediately jump states after fault recovery
                last_state_change = time.time()
            else:
                # --- B. STATE MACHINE ---
                ...
                if now - last_can_time >= (1.0 / CAN_HZ):
                    # Redundantly send limits to keep DSP happy
                    try:
