...
# FIX: Specify the exact prefix for Calex telemetry logs
# Define the specific file patterns you want to match
patterns = ['calex_cycle_log_*.csv', 'calex_log_*.csv', 'calex_charge_log_*.csv', 'calex_discharge_log_*.csv']

# Gather all matching files into a single list
list_of_files = [file for p in patterns for file in glob.glob(os.path.join(LOG_DIR, p))]

if not list_of_files:
    print("No Calex telemetry logs found. Checking directory permissions or filenames...")
    exit(1)
